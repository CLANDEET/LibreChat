...
/**
 * 워드마크는 사이드바·헤더·로그인에서 같은 표기여야 하고, 색 때문에 `Prism` 이
 * 별도 span 으로 쪼개져 있다. 스크린리더가 "BKL" 과 "Prism" 을 따로 읽지 않도록
 * aria-label 로 한 덩어리를 만들어 둔 상태라, 그 구조가 깨지면 접근성만 조용히
 * 나빠지고 화면상으로는 티가 안 난다.
 */
describe('PrismWordmark', () => {
  ...
  it('기본 tone 은 accent — 사이드바·헤더가 이걸 쓴다', () => {
    const { container } = render(<PrismWordmark />);
    expect(container.querySelector('.prism-spectrum-text')).toBeNull();
    expect(container.querySelector('.text-\\[color\\:var\\(--prism-accent\\)\\]')?.textContent).toBe(
      'Prism',
    );
  });

  it('tone="spectrum" 은 Prism 에만 그라디언트를 입힌다', () => {
    const { container } = render(<PrismWordmark tone="spectrum" />);
  ...
  it('tone="inherit" 은 어떤 색도 얹지 않는다', () => {
    const { container } = render(<PrismWordmark tone="inherit" />);
    expect(container.querySelector('.prism-spectrum-text')).toBeNull();
    expect(container.querySelector('[class*="prism-accent"]')).toBeNull();
    expect(container.textContent).toBe('BKL Prism');
  });
